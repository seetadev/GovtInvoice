...
Aspiring.msn.numtextrows = 1;


Aspiring.msn.isFinancials = function(tab, type) {
    return false;
};


Aspiring.msn.xbrlmap = {
...
Aspiring.msn.urlbase = "http://investing.money.msn.com/investments";


Aspiring.msn.getUrl = function(ticker, type, period) {
    var maptype = { "Income":"/stock-income-statement/",
 		    "Balance":"/stock-balance-sheet/",
		    "CashFlow":"/stock-cash-flow/"
    };
    url = Aspiring.msn.urlbase+maptype[type]+"?symbol="+ticker+"&stmtView="+period;
    console.log("loading ticker "+ticker);
    console.log("url is:"+url);
    return url;
};

Aspiring.msn.getTable = function(table) {
    //console.log(table);
    var tab = new Array();
	...
	if (innertables.length > 0) {
		...
		val = Aspiring.cleanString(val);
                //normalize later
		//if (j == 0) {
		//    var newval = Aspiring.xbrlmap[val];
		//    if (newval != null) {
		//       val = newval;	
		//    }
		//}
...
Aspiring.msn.analyzeStatement = function(text) {
	...
	tab = Aspiring.msn.getTable(tables[1]);
	//console.log("in 2");
...
Aspiring.msn.normalizeTable = function(tab, type) {
// convert the MSN table data into a normalized form
//  
    var map = {};

    for (var i=0; i<tab.length; i++) {
	var ind = Aspiring.msn.xbrlmap[tab[i][0]];
	if (ind == null) {
	    ind = tab[i][0];
	} else {
	    tab[i][0] = ind;
